...
GFDLengthExtender.begin = () => {
    ...
    Game.registerMod(GFDLengthExtender.name, GFDLengthExtender);//MOVE IT ABOVE OR BELOW AS NEEDED 

    if (Game.modSaveData.GFDLengthExtender) {
        
        if (JSON.parse(Game.modSaveData.GFDLengthExtender).GFDLENGTH) {
			GFDLengthExtender.GFDLENGTH =
				JSON.parse(Game.modSaveData.GFDLengthExtender).GFDLENGTH
        }

	} else if (GFDLengthExtender.GFDLENGTH === undefined) {
		
        GFDLengthExtender.GFDLENGTH = 2;
	...
	GFDLengthExtender.load = (str) => {
		...
		if (!("GFDLENGTH" in saveData)) {
			console.error("No predefined GFDLENGTH");
		}
		GFDLengthExtender.GFDLENGTH = saveData.GFDLENGTH;
	};
	...
	//     }
	// });
	Game.Objects["Wizard tower"].minigame.spells["gambler's fever dream"].win = function () {
